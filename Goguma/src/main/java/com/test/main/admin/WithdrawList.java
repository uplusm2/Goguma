...
@WebServlet("/admin/withdrawlist.do")
public class WithdrawList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<WithdrawDTO> list = dao.list(map);
		
		for(WithdrawDTO dto : list) {
			//날짜 년월일로 자르기
		...
		if (n == 1) {
			...
			pagebar += String.format("<li><a href='/goguma/admin/withdrawlist.do?page=%d' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>", n-1);				
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if(n == nowPage) {
				...
				pagebar += String.format("<li><a href='/goguma/admin/withdrawlist.do?page=%d'>%d</a></li>", n, n);								
			}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/admin/withdrawlist.jsp");
