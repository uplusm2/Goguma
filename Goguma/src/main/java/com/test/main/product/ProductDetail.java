...
//		req.setAttribute("imglist", imglist);
		...
		for (ProductDTO pdto : list) {
			...
			//날짜 자르기
			if (pdto.getRegdate().startsWith(strNow)) {
				pdto.setRegdate(pdto.getRegdate().substring(11));
			} else {
			...
			//제목이 길면 자르기
			if (pdto.getName().length() > 10) {
				pdto.setName(pdto.getName().substring(0, 10) + "..");
			}
			...
			if (searchmode.equals("y") && column.equals("subject")) {
				...
				pdto.setName(pdto.getName().replace(word, "<span style='background-color:yellow;color:tomato;'>" + word + "</span>"));
			}
