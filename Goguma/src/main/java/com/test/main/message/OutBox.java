...
//			}
	...
	private String getPagebar() {
		...
		if (n == 1) {
			...
			pagebar += String.format("<li class='previous'><a href='/goguma/message/outBox.do?page=%d' aria-label='Previous'><span class='glyphicon glyphicon-menu-left'></span></a></li>", n-1);
		}
		...
		while (!(loop > pageSize || n > totalPage)) {
			if (n == nowPage) {
				...
				pagebar += String.format("<li><a href='/goguma/message/outBox.do?page=%d'>%d</a></li>", n, n);
			}			
			loop++;
			n++;
		}
		...
		if (n > totalPage) {
			...
			pagebar += String.format("<li class='next'><a href='/goguma/message/outBox.do?page=%d' aria-label='Next'><span class='glyphicon glyphicon-menu-right'></span></a></li>", n);
		}
