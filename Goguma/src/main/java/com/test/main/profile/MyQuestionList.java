...
@WebServlet("/profile/myquestionlist.do")
public class MyQuestionList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		totalCount = dao.getMyQuestionListCount(map);
		...
		if (n == 1) {
			...
			pagebar += String.format("<li><a href='/goguma/profile/myquestionlist.do?page=%d&search=%s' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>", n-1, search);
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += String.format("<li><a href='/goguma/profile/myquestionlist.do?page=%d&search=%s'>%d</a></li>", n, search ,n);
			}			
		...
		if (n > totalPage) {
			...
			pagebar += String.format("<li><a href='/goguma/profile/myquestionlist.do?page=%d&search=%s' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>", n, search);
		}
		...
		ArrayList<CenterDTO> list = dao.myQuestionList(map);
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/profile/myquestionlist.jsp");
