		...
		<section>
			<div id="main-box">
				...
				<div id="main-right">
					...
					<div id="price-tab">
						...
						<c:if test="${dto.is_completion == 'y'}">
							<button type="button" class="gray-btn">판매완료</button>
						</c:if>
						<c:if test="${dto.is_completion == 'n'}">
							<c:choose>
								<c:when test="${dto.is_auction == 'y'}">
									<button type="button" class="red-btn">경매중</button>
								</c:when>
								<c:when test="${dto.is_auction == 'n'}">
									<button type="button" class="red-btn">판매중</button>
								</c:when>
							</c:choose>
						</c:if>
                ...
                <c:if test="${check eq '1'}">
					...
					<div class="product-address">${adto.sido}${adto.sgg}
						${adto.emd} / ${tdto.name}</div>

					<div
						style="overflow: auto; width: 500px; height: 300px; padding-top: 15px">

						<p class="main-text">${dto.content}</p>
					</div>
				...
				<div id="bottom-box">
					...
					<c:if test="${dto.is_completion eq 'n'}">
						...
						<c:if test="${dto.is_auction eq 'y'}">
							...
							<button type="button" class="mid-btn3">판매자정보</button>
							<button type="button" class="mid-btn2"
								onclick="location.href='/goguma/product/bidadd.do?seq=${dto.seq}';">
								입찰하기</button>
						</c:if>
						<c:if test="${dto.is_auction eq 'n'}">
							...
							<button type="button" class="mid-btn3">판매자정보</button>
							<button type="button" class="mid-btn2"
								style="visibility: hidden;">입찰하기</button>
						</c:if>
					</c:if>
					...
					<input type="button" class="mid-btn4" value="댓글"
						style="visibility: hidden;"> <input type="button"
						class="mid-btn5 btn primary-btn" value="입찰내역"
						onclick="location.href='/goguma/product/bidList.do?seq=${dto.seq}';">
			...
			<div class="list con" style="margin-left: 30px;">
				<ul class="row">
					...
					<c:forEach items="${list}" var="dto">
						...
						<li class="cell">
							<div class="img-box">
								<a href="/goguma/product/productDetail.do?seq=${dto.seq}"><img
									src="/goguma/files/product/${dto.main_img}"
									onerror="this.src='/goguma/files/product/defaultimg.jpg';"
									alt=""></a>
							</div>
							<div class="product-name">
								<a href="/goguma/product/productDetail.do?seq=${dto.seq}">${dto.name}</a>
							</div>
